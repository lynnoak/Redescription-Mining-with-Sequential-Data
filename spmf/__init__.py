...
write_temp_input_file:
    add the file close for correct
to_pandas_dataframe:
    add the support id for output
"""
        ...
        for pattern_sup in self.patterns_:
            ...
            if len(sup) > 3:
                if sup[2] == "#SID:":
                    ...
                    patterns_dict_list.append({'pattern': pattern, 'sup': int(sup_n), 'sid': sid})
                else:
                    patterns_dict_list.append({'pattern': pattern, 'sup': int(sup_n)})
            else:
                patterns_dict_list.append({'pattern': pattern, 'sup': int(sup_n)})
